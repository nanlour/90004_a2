...
class MuscleFiber(Agent):
    ...
    def perform_daily_activity(self):
        ...
        self.fatigue += 0.1  # Simulating daily activity increasing fatigue

    def lift_weights(self):
        if random.random() < (self.model.intensity / 100) ** 2:
            damage = random.uniform(0.3, 0.7)  # Simulating muscle microdamage
            self.fiber_size -= damage
            self.catabolic_hormone += math.log10(self.fiber_size) * 44
            self.anabolic_hormone += math.log10(self.fiber_size) * 55
            self.fatigue += 0.5  # Increase fatigue due to lifting weights

    def sleep(self):
        repair_rate = 0.05 * self.fiber_size
        self.fiber_size += repair_rate  # Muscle repair during sleep
        ...
        self.fatigue = max(0, self.fatigue - 0.3)  # Reducing fatigue during sleep

    def regulate_hormones(self):
        if self.anabolic_hormone > 100:
            self.catabolic_hormone -= 0.1 * (self.anabolic_hormone - 100)
        if self.catabolic_hormone > 100:
            self.anabolic_hormone -= 0.1 * (self.catabolic_hormone - 100)
...
class MuscleModel(Model):
    def __init__(self, width, height, lift_weights, hours_of_sleep, intensity, days_between_workouts, slow_twitch_fibers):
        ...
        self.datacollector = DataCollector(
            model_reporters={
                "Muscle Mass": lambda m: m.muscle_mass,
                "Anabolic Hormone": lambda a: a.anabolic_hormone_mean,
                "Catabolic Hormone": lambda c: c.catabolic_hormone_mean,
                "Average Fatigue": lambda f: sum(a.fatigue for a in f.schedule.agents) / len(f.schedule.agents)
            }
        )
